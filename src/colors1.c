...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/02 15:51:42 by jlange            #+#    #+#             */
/*   Updated: 2017/04/02 15:51:43 by jlange           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int		ft_color_tmp3(t_mlx *mlx, t_remap info, int x, int y)
{
	...
	z = (mlx->map[y][x + 1].z > mlx->map[y][x].z) ?
	ft_remap2(mlx->map[y][x + 1].z, info) :
...
static inline int		ft_color_tmp1(t_mlx *mlx, t_remap info, int x, int y)
{
	...
	if (mlx->color % 8 == 0)
		...
		info.out_max = 0xF;
	z1 = ft_remap2(mlx->map[y][x + 1].z, info);
